...
class GetBlockPage(object):
    def __init__(self, server, session):
        ...
        self.available_blocks_label = Label(self.get_block_page_root, text="Available Blocks")
        self.available_blocks_label.grid(row=0, column=0)

        self.selected_blocks_label = Label(self.get_block_page_root, text="Selected Blocks")
        self.selected_blocks_label.grid(row=0, column=1)

        self.available_blocks_box = Listbox(self.get_block_page_root, width=31, height=20)
        self.available_blocks_box.grid(row=1, column=0, rowspan=9)
        self.available_blocks_box.bind('<Double-Button-1>', self.select_block)

        self.selected_blocks_box = Listbox(self.get_block_page_root, width=31, height=20, selectmode=MULTIPLE)
        self.selected_blocks_box.grid(row=1, column=1, rowspan=9)

        self.delete_selected = Button(self.get_block_page_root, text="Delete Selected", command=self.delete_selected)
        self.delete_selected.grid(row=10, column=1)

        self.delete_all = Button(self.get_block_page_root, text="Delete All", command=self.delete_all)
        self.delete_all.grid(row=11, column=1)

        self.get_blocks_button = Button(self.get_block_page_root, text="Get Blocks", command=self.get_blocks)
    ...
    def load_block_list_from_server(self):
        if not self.server.get_block_list_url:
            showwarning("Load Config", "You need to load the config.json file first")
            return
        self.server.get_block_list_from_server()

        i = 0
        for blockID, info in self.session.block_list_from_server.iteritems():
